...
// Get evaluations for a student with status info
export const getStudentEvaluations = query({
  args: {
    studentId: v.id("users"),
  },
  handler: async (ctx, args) => {
    try {
      ...
      let status: "waiting" | "available" | "no_lessons" = "available";
      let nextEvaluationDate: Date | null = null;
      let encouragementMessage: string | null = null;

      // Only check for waiting status if there are evaluations
      if (evaluations.length > 0) {
        // Check if there's an evaluation for the current month
        const hasCurrentMonthEval = evaluations.some(
          (evaluation) =>
        ...
        const mostRecentEval = evaluations[0];

        if (hasCurrentMonthEval) {
          status = "waiting";
          // Next evaluation would be next month
          nextEvaluationDate = new Date(currentYear, currentMonth + 1, 1);

          encouragementMessage =
            "Great work this month! Keep practicing and your teacher will evaluate your progress again next month. Remember, consistent practice is the key to improvement!";
        } else if (mostRecentEval) {
          // Check if it's been less than a month since last evaluation
          const daysSinceLastEval = Math.floor(
            (now.getTime() - mostRecentEval.createdAt) / (1000 * 60 * 60 * 24),
          );

          if (daysSinceLastEval < 25) {
            status = "waiting";
            const lastEvalDate = new Date(mostRecentEval.createdAt);
            nextEvaluationDate = new Date(
              lastEvalDate.getFullYear(),
              lastEvalDate.getMonth() + 1,
              lastEvalDate.getDate(),
            );

            encouragementMessage =
              "You're making great progress! Your teacher will complete your next evaluation soon. Keep up the excellent work!";
          }
        }
      }

      return {
        evaluations: evaluationsWithTeacher,
        status,
        nextEvaluationDate: nextEvaluationDate?.getTime() || null,
        encouragementMessage,
