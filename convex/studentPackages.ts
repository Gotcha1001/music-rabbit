...
// NEW: Admin credits free draw package (no payment, manual email by staff)

export const adminCreditFreePackage = mutation({
  args: {
    studentId: v.id("users"),
    packageType: v.string(), // e.g., "monthly-draw-win"
    lessonsPerWeek: v.number(),
    minutesPerLesson: v.number(),
    totalMinutesPerMonth: v.number(),
  },
  handler: async (
    ...
    let assignedMonth: string;

    // Find the first month (current or future) without an active package
    while (true) {
      assignedMonth = format(targetDate, "yyyy-MM");
      ...
      const existing = await ctx.db
        ...
        .filter((q) =>
          q.and(
            q.eq(q.field("status"), "active"),
            q.eq(q.field("currentMonth"), assignedMonth),
          ),
        )
        .first();
    ...
    const expiryDate = lastDayOfMonth(targetDate).getTime();

    const packageId = await ctx.db.insert("studentPackages", {
      studentId: args.studentId,
      packageType: args.packageType,
      lessonsPerWeek: args.lessonsPerWeek,
      minutesPerLesson: args.minutesPerLesson,
      ...
      monthlyPrice: 0, // Hardcoded free
      currentMonth: assignedMonth,
      purchaseDate: Date.now(),
      expiryDate,
      paymentId: `free-draw-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
      status: "active",
      stripeSubscriptionId: undefined,
    });

    return { success: true, packageId, assignedMonth };
  },
});
