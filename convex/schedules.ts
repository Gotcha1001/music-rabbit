...
//     const interval = 30; // 1 min increments for precision
...
//     // For each day, calculate available slots
//     for (const date of Object.keys(availability)) {
//       const availableTimes = await ctx.runQuery(
...
// // NEW: Student self-reschedule same-day
// export const studentSelfRescheduleSameDay = mutation({
//   args: {
//     scheduleId: v.id("schedules"),
//     lessonId: v.string(),
//     newTime: v.string(),
//     reason: v.optional(v.string()),
//   },
//   handler: async (ctx, { scheduleId, lessonId, newTime, reason }) => {
//     const identity = await ctx.auth.getUserIdentity();
//     if (!identity) throw new Error("Unauthorized");
...
//     if (schedule.date !== nowDate) {
//       throw new Error("Can only reschedule same-day lessons");
//     }

//     // Check 2-hour advance
//     const now = Date.now();
//     const [year, month, day] = schedule.date.split("-").map(Number);
//     const [hour, minute] = lesson.time.split(":").map(Number);
//     const lessonDateTime = new Date(
...
//       minute,
//     ).getTime();
//     const hoursToLesson = (lessonDateTime - now) / (1000 * 60 * 60);
//     if (hoursToLesson < 2) {
...
//     // Update lesson time
//     const updatedLessons = [...schedule.lessons];
//     updatedLessons[lessonIndex] = { ...lesson, time: newTime };
...
//       content: `Student rescheduled lesson from ${lesson.time} to ${newTime} on ${schedule.date}. Reason: ${reason || "None"}`,
//       timestamp: Date.now(),
//       isRead: false,
//       deletedBy: [],
//     });
...
// ============================================================================
// STUDENT: SAME-DAY RESCHEDULE (2-HOUR ADVANCE)
// ============================================================================
export const studentSelfRescheduleSameDay = mutation({
  args: {
    scheduleId: v.id("schedules"),
  ...
  handler: async (ctx, { scheduleId, lessonId, newTime, reason }) => {
    ...
    if (!isWithinWorkingHours(schedule.date, newTime, teacher.timezone)) {
      throw new Error(
        "New time is outside teacher's working hours (10:00-17:00)",
      );
    }
    // ✅ CHECK CONFLICTS WITH 1-MINUTE GAP
    const occupied: { startMin: number; endMin: number }[] = [];
    for (let i = 0; i < schedule.lessons.length; i++) {
      if (i === lessonIndex) continue; // Skip current lesson
      const l = schedule.lessons[i];
    ...
    if (overlaps) {
      throw new Error(
        "New time conflicts with another lesson (remember 1-minute gap rule)",
      );
    }
    // ✅ UPDATE LESSON TIME
    const updatedLessons = [...schedule.lessons];
